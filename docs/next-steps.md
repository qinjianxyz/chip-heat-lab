...
The transient value-loop and knowledge page upgrade has landed. The only
adjacent addition permitted before recording is a bounded power-delivery proxy
that uses the same floorplan and power map. If that proxy is merged and green,
move directly to recording instead of opening another model lane.

## P0 Before Submission

1. Record the final demo video after the power-delivery proxy is either merged
   and redeployed or explicitly deferred.
